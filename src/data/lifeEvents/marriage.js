...
// license, name change, insurance windows, beneficiaries — not wedding
// planning (venues and guest lists are not Mitzy's lane). Due dates anchor
// to the wedding date, which can be in the future or the past; for users
// adding the event after the wedding, the before-the-day tasks are moot and
// skipped entirely. Weddings are good news: confetti stays on.
...
// Gates from intake answers.
const NAME_CHANGE_ONLY       = new Set(['name-change']);
...
const BUNDLE = [
  ...
  { id: 'name-change',          phase: 'AFTER', cat: 'finance',    label: 'Update your name (Social Security, ID, passport, accounts)', stakes: 'high', assistType: 'guidance' },
...
function passesGates(t, answers) {
  if (NAME_CHANGE_ONLY.has(t.id) && !answers?.nameChange)                     return false;
  if (COMBINING_ONLY.has(t.id) && !answers?.combiningFinances)                return false;
  ...
  intake: {
    steps: [
      ...
      },
      {
        ...
        fields: [
          { key: 'nameChange',               label: 'Is either of you changing your name?' },
          { key: 'combiningFinances',        label: 'Are you planning to combine finances (joint accounts)?' },
