...
| `task.guidance` field | Populated for the base task library and all six life event bundles (step-by-step, inline-numbered text, e.g. `"1. ... 2. ..."`). Custom/AI-generated tasks have none and rely on the Assist button (live AI guidance) instead — a static script doesn't fit every situation. `task.why` is populated across the base library and all life event bundles. |
| Provider data | Claude-generated, no verification. |
| Jurisdiction guidance | Live. `jurisdiction` and `deadline` run Anthropic's server-side web search on Sonnet 5 and cite the pages they used, so county filing fees and deadlines come from the real clerk's page, streamed back as they're written. Search is best-effort — a failure, refusal, or empty answer silently falls back to the no-tools Haiku path with the conservative prompt, so a bad search degrades to vaguer guidance rather than an error. Answers are not verified against the source beyond the model's own citation. The lookup still runs inside a 60s function budget; streaming means an overrun costs the user the tail of an answer rather than the whole request, but it doesn't remove the ceiling. |
...
3. Wire up real billing (Stripe or similar) and enforce the Mitzy Pro paywall on AI features to match the landing page's pricing claims. Web-search assist is the most expensive per-request feature and is currently ungated.
