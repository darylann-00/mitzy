...
- **AssistPanel** — Full-screen overlay. Provider/script/deadline/guidance modes. Caches 7 days. Provider mode passes `task.searchQuery` (if set) to `/api/providers` so Places queries are task-appropriate rather than using the raw label.
...
**2026-03-30:** Added zip → climate region mapping and region-aware seasonal task visibility. New `src/utils/climateRegion.js` exports `getClimateRegion(zip)` (9 regions: pacific_nw, california, southeast, florida, northeast, midwest, southwest, mountain, gulf) and `REGION_TASK_ADJUSTMENTS` (per-region overrides for seasonStart/activeMonths on 14 seasonal tasks). `isWindowActive(task, region)` in `taskLogic.js` now accepts a region param; `seasonStart: false` or `activeMonths: []` hides a task in that region. `App.js` derives region from `profile.zip` and passes it through. Key outcomes: FL users never see winterize-faucets or winter emergency tasks; Seattle users get hurricane tasks hidden and wiper/sump windows shifted to Oct; Minneapolis users get the full Aug–Mar winter prep window.
