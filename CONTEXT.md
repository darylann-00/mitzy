...
| AI | Claude Haiku 4.5 via `/api/assist` + `/api/generate-task`; Claude Sonnet 5 + web search for the `jurisdiction` and `deadline` assist types only |
| Deployment | Vercel |
...
- **AllView** — Three urgency groups (swipeable cards). Category filter chips. Due-only toggle. `GroupDivider` between groups. Category icon tile on each card. When a life event is active, its tasks are pulled out of the normal category chips (Home/Car/etc.) and live behind their own dynamic "[event name]" chip instead, positioned right after "All" — selecting it runs the same Needs attention/Coming up/All good grouping as any other category, rather than a special always-expanded box. Collapsible "Snoozed" section between "All good" and "Explore". "X tasks to explore" accordion section at bottom for `unknown`-status tasks with inline chip picker. `paddingBottom: 160px`.
...
- **AssistPanel** — Full-screen overlay. Provider/script/deadline/guidance/guidance_companies/jurisdiction modes. Caches 7 days for most types, 30 days for the web-search-backed ones (`jurisdiction`, `deadline`) since a live lookup costs real money and court fees move yearly, not weekly (currently v16 — bump `ASSIST_CACHE_PREFIX` whenever a prompt change should reach users who already have a cached answer, or the 30-day types keep serving the old wording for a month). Every prompt is built by `buildAssistPrompt` (async), which resolves the profile zip to a real county and state via `resolveLocation()` in `src/utils/geo.js` — zip → county FIPS → county + state name, from `public/data/zip-to-fips.json` plus `public/data/fips-to-county.json` (built by `scripts/build-county-names.mjs` from the same Census crosswalk the hazard data uses; re-run it manually when Census publishes a new vintage). The prompt says "in Travis County, Texas (zip 78704)" when it resolves and falls back to the bare zip, then to "in my area", when it doesn't. `jurisdiction` mode is for tasks whose procedure is set by state or local law; it and `deadline` are the two types that run a live web lookup, so their prompts come in two variants — `buildAssistPrompt(task, profile, { search: true })` names the county and state, tells the model to prefer the responsible agency's own site over law-firm and aggregator pages, and requires a markdown source link for every fee, deadline, and phone number; the default variant forbids stating any of those as fact and names the office to call instead. Both search variants also require a closing **Where to go** link to the page holding the forms or dates — source citations alone were owed only when the model stated a fee or a date, so an answer that carefully stated neither came back with no link at all. `shouldUseSearch(task)` is what decides the path: it ORs the assist type against a per-task `searchAssist: true` flag, so a single high-value `guidance` task can be promoted to the live-lookup model without changing its assistType or rendering (nothing sets the flag yet). The client posts `prompt` + `fallbackPrompt` + `assistType` + `search`, and the server ORs the last two the same way. The two search-backed types **stream** their answer back as line-delimited JSON (status/text/reset/done/error events) since the lookup behind them is slow enough to have timed out in production twice before the server took ownership of its own 35s deadline — the panel shows what the server is actually doing ("Looking up official sources..." → "Reading what it found...") and then renders the markdown as it arrives, instead of a loader for the whole wait. Everything else still gets one `{ text }` JSON body. The answer is only written to the cache on the stream's `done` event, so a truncated answer is never stored or shown as finished. Any new `assistType` must be added to the render gate in `AssistPanel.jsx` or the panel fetches successfully and renders blank; if it's also search-backed, the gate has to accept the `streaming` status too or the text renders only after the stream closes. Provider mode passes `task.searchQuery` (if set) to `/api/providers` so Places queries are task-appropriate rather than using the raw label. Provider cards show condensed weekly hours (Claude-formatted from Places `weekdayDescriptions`), review count under star rating, address links to Google Maps, blurbs with **bold** key phrases. `guidance_companies` mode returns JSON with guidance markdown + top 3 national companies (no aggregators); renders `renderMarkdown` + `CompanyCard` rows with external link icon. `renderMarkdown` handles ##headers, bullets, numbered lists with nested sub-bullets, tables, horizontal rules, bold, and auto-linked URLs. Every prompt except `script` carries link rules from `src/data/officialLinks.js` (see "Official links" below): on the no-tools path the model gets that task's verified links plus an explicit ban on writing any other URL — it cannot check one, and a broken .gov link on a divorce or death task is worse than no link. On the search path the same list is offered as a floor, with the page it actually found taking priority. The ban is scoped to the prose for `guidance_companies`, which returns real company URLs in its own JSON. `PulseLoader` cycles through contextual messages per `assistType` every 2.5s — 3 for most, 4 for the search-backed types since those are slow and three would loop; providers uses `task.searchQuery || task.label` for specificity. Once a search-backed stream starts reporting, those canned messages are replaced by what the server says it's actually doing, and the loader disappears entirely as soon as the first prose arrives.

- **Official links** — `src/data/officialLinks.js` holds ~22 hand-verified URLs (`OFFICIAL_LINKS`) plus a task→links map. Every entry was opened and confirmed before it went in; nothing state-specific lives here unless the page is itself a 50-state directory (CDC vital records, USAGov DMV finder, vote.gov, LawHelp.org forms, LSC legal aid, NAUPA unclaimed property) — those directories are what let the no-search path give a useful link without knowing the user's state. `linksForTask(task)` matches a life event's `eventBundleKey` or a base task's `id`, falling back to a longest-first prefix match so generated per-car/kid/pet tasks (`car-reg-{slug}`) resolve too. `officialLink(id, text)` renders one as markdown for static guidance copy, so a URL is written in exactly one place; it degrades to plain text rather than emitting a broken link if the id is wrong. Static `guidance` strings in `tasks.js` and the life event bundles use it wherever the copy already pointed at a site — those are rendered by `renderMarkdown.jsx`, which supports `[label](url)` and drops any non-http scheme. Links are re-checked with `node scripts/check-official-links.mjs` (not in CI — these are third-party sites, several 403 any non-browser request, and a flaky government site must never block a deploy). Re-run it roughly annually.
...
- **Life events** — Contextual task bundles triggered by major life changes. Six events ship: "New baby" (`newBaby.js`), "Getting married" (`marriage.js`, admin only — license, insurance windows, beneficiaries — not wedding planning), "Changing your name" (`nameChange.js`, its own event since it isn't unique to marriage — also fits divorce or a personal decision), "Divorce or separation" (`divorce.js`, whose legal steps branch on an intake `representation` question — attorney / mediation / self / undecided — since plenty of people divorce without a lawyer; self-filers get the procedural tasks an attorney would otherwise absorb (court forms, simplified-process eligibility, service + proof of service, final hearing) plus a legal-aid check in place of the attorney consult, mediation gets its own mediator/agreement track, and tasks shared across paths carry a `guidanceFor` map that swaps in path-specific steps so a self-filer is never told "your attorney will handle this" — resolved and stripped in `tasksForIntake`; an event with no `representation` answer falls back to `undecided`), "Loss of a loved one" (`lossOfLovedOne.js`), and "Moving" (`moving.js`, 23 tasks across BEFORE/AFTER phases — address changes, utilities, providers, legal paperwork; two separate boolean gates keep out-of-state legal tasks and provider-finding tasks independent so cross-state short-distance moves and same-state long-distance moves both get the right subset; profile fields `hasKids`/`hasPets`/`hasCar` seeded from profile, `ownsHome` gates listing vs. give-notice, driver's license always shows with label swap for out-of-state), registered in `src/data/lifeEvents/index.js`. Each def carries its bundle, phase order/labels, intake step config, and `tasksForIntake`/`retroactiveCandidates` generators; sad events set `suppressCelebration` so no confetti fires on task completion. Every bundle task has a plain-language `why` and a step-by-step `guidance` field (both populated across all six defs — these drive the task detail view's "Why it matters" and "What to expect" cards; without them the UI falls back to generic filler copy that reads oddly for anything outside home-maintenance tasks). Eighteen tasks whose procedure is set by state, county, or city government — divorce petition and the self-filing steps around it (forms, service, final hearing, simplified-process eligibility, legal aid, entering a mediated agreement), marriage license and certificates, driver's license (name change and move), voter registration (name change and move), vehicle registration, pet licensing, death certificates, estate settlement, multi-state taxes — carry `assistType: 'jurisdiction'` rather than `'guidance'`; federally uniform tasks (Social Security card, passport, W-4) deliberately stay `'guidance'`. Guidance copy in these bundles must not assert a government-set fee, deadline, or waiting period as national fact — those vary by jurisdiction and Mitzy cannot verify them, so the copy names the office to ask instead (market-price estimates and federal constants are fine). Every generated task also gets a computed due date — event anchor date (baby due date, wedding date, move date, date of passing, or today for divorce) + per-phase/per-task offset, clamped ≥7 days out (`eventDates.js`) — stored on `custom_tasks.due_date`, so event tasks flow through normal due/coming-up status, HomeView focus, and weekly check-ins at the right time; a user-set `task_records.due_date` overrides the default. `useLifeEvents` manages event state in Supabase (`life_events` + `custom_tasks`). Intake boolean fields can set `allowUnsure: true` (currently marriage's "combining finances" question) to offer a third "Not sure yet" answer alongside Yes/No, stored as the string `'unsure'` — gating logic must compare `=== true` rather than truthy, since `'unsure'` is a non-empty string. Unresolved "not sure yet" decisions on the active event surface in ProfileView's Life events card under "Still deciding," with Yes/No buttons that call `lifeEvents.resolveEventAnswer(eventId, key, value)` — this patches `intake_answers` in Supabase and adds any tasks the new answer newly unlocks (matched against existing tasks by `eventBundleKey` so nothing is ever duplicated). That surface only renders boolean fields, so a choice-step answer left open — divorce's `representation: 'undecided'` — has no in-app way to be revisited yet, even though `resolveEventAnswer` itself is key-agnostic and would handle the switch. `LifeEventNudge` yellow card appears on HomeView in two variants: "discovery" (introduces the feature) and "wrapup" (fires when all tasks for an active event are complete). New-baby keeps its bespoke `NewBabyIntake`; other events use the config-driven `GenericEventIntake` (step types date/choice/booleans + shared retro "already handled?" checklist + confirm). Facts Mitzy already knows (e.g. `profile.hasKids` gating divorce kid-tasks) are seeded into intake answers (`hasKids`, `hasPets`, `hasCar`), never re-asked. Estate tasks in the loss event are gated on the user actually settling affairs. Tasks are ID-prefixed (`lf-{type}-{id}-{taskId}`) so multiple instances don't collide. Users can add their own custom tasks to an active event from the sparkle FAB: a dynamic life event category chip appears in the category row when an event is active — selecting it links the task to the event, hides the frequency section (forced one-time), and shows an "Add to [event name]" button. In AI/brain-dump mode, `/api/generate-task` receives active event context and tags tasks as `lifeEventRelevant: true` when they logically belong to the event; tagged tasks are auto-linked on save. Saved event tasks get `lifeEventId`, `oneTime: true`, and `suppressCelebration` set automatically; user-added tasks use normal `genTaskId()` IDs (not the `lf-` bundle prefix) and sort after bundle tasks. One event can be active at a time — starting a second event (e.g. Name change right after Marriage) requires completing or dismissing the first.
...
- **Task Creator** — Sparkle FAB opens `TaskCreator` full-screen overlay with two modes: "Mitzy magic" (AI, default) and "Do it myself" (manual), switched via a light segmented control (surface-colored track, white active thumb) at the top of the body. AI mode: brain-dump card (Righteous "Brain dump" micro-label, borderless textarea, brand focus ring, helper footer, tappable example chips below) → `/api/generate-task` (Haiku 4.5) auto-detects one vs many tasks. Single task → `TaskConfirmCard` review. Multiple tasks → `BrainDumpReview` compact checklist (checkbox, `CategoryTile` icon, label, frequency + category text, tap-to-edit via `TaskConfirmCard` with custom button labels). Batch save calls `addCustomTasksBulk`. Manual mode: white-card form (Task / Category / How often / How important) matching `TaskConfirmCard` styling — `CategoryTile` category buttons incl. the custom-task-only "Other" category (four-dot logo icon; in `CAT_META`, `CAT_ICON_CONFIG`, AllView chips, and the AI cat enum), a Recurring/One time toggle with an "Every [N] [days|weeks|months|years]" sentence control (no preset chips), low/medium/high stakes chips, no API call. Feature test: `tests/e2e/task_creator.spec.js` (manual-mode save, fully mocked Supabase writes). AI features: assumption chips cycle on tap and re-fire the API (debounced 400ms, AbortController cancels in-flight). T2 tasks show a silent DIY toggle. T3 locked. T4 (crisis) shows refusal + hotline. T0 (parse failure) shows manual fallback. Prompt limit 2000 chars, max_tokens 4000. Rate limited at 20 req/hr per user (`rl:gentask` prefix in Upstash). `useProfile` exposes `addCustomTask`, `addCustomTasksBulk`, and `removeCustomTask`.
...
- **Landing page (returning vs new)** — `LandingPage` is the first screen on cold launch (before `SlimOnboarding`). Marketing page (hero with category eyebrow + product screenshot, task-chip section, "How it works" 3 steps, benefit cards, pricing — Free vs. Mitzy Pro at $4.99/mo, closing CTA) with two entry points: "Start my free trial" or "Sign in" in the nav. Choice persisted to `WELCOME_CHOICE_KEY` (`mitzy-welcome-v1`) as `'new'` or `'returning'`. `'returning'` skips onboarding and goes straight to `LoginGate` → server profile loads from Supabase into local state. `'new'` keeps the original onboarding → priority-setup → login flow. If a returning user signs in with no server profile (typo, wrong account), `App.js` flips them to `'new'` and routes through onboarding. Welcome key is included in `USER_KEYS`, so reset/sign-out clears it. Pricing tiers are marketing copy only — the app does not yet enforce the Free/Pro split (see "What's Mocked / Incomplete"). The page is also the surface Google's OAuth branding reviewers read, so purpose is stated without turning the page into prose: a "Household task manager" eyebrow pill above the h1, and a wrapped row of real task-library chips (HVAC filter, car registration, kids' checkup…) under the heading "The stuff you're supposed to remember" with the plain sentence "Mitzy is a household task manager. It knows hundreds of tasks…" as its subhead. There is deliberately no separate Google/privacy section — calendar behaviour is described inside the "Works with your calendar" benefit card instead. That copy must stay accurate in both directions: Mitzy reads events for matching **and** creates them via `api/schedule.js` (`POST calendars/primary/events`), so neither the landing page, `index.html`, `public/privacy.html`, nor `public/terms.html` may claim read-only calendar access. Benefit cards reuse the app's own `CategoryIcons` (not emoji) so the page reads as the same product. The footer carries the data-transparency paragraph (household details, Google sign-in, optional calendar read + write) and links the privacy policy at its absolute `https://mitzy.io/privacy.html` URL, which must stay byte-identical to the URL configured on the OAuth consent screen. `index.html` carries the same summary as a plain-HTML `#boot-fallback` block inside `#root` for crawlers and no-JS visitors. It is hidden by a plain `#boot-fallback { display: none }` rule and revealed by a `<noscript>` override — deliberately CSS-only, because `vercel.json`'s `script-src` has no `'unsafe-inline'`, so any inline `<script>` in `index.html` is blocked in production while still running under `vite preview`. Covered by `tests/e2e/landing.spec.js`, which aborts the app bundle so the no-flash assertion holds with zero app JS.
...
| `task.guidance` field | Populated for the base task library and all six life event bundles (step-by-step, inline-numbered text, e.g. `"1. ... 2. ..."`). Roughly 20 of those strings carry a verified markdown link via `officialLink()`; the rest are prose only, and adding a link means adding a verified entry to `officialLinks.js` first — never a URL typed inline. Custom/AI-generated tasks have none and rely on the Assist button (live AI guidance) instead — a static script doesn't fit every situation. `task.why` is populated across the base library and all life event bundles. |
| Provider data | Claude-generated, no verification. |
| Jurisdiction guidance | Live. `jurisdiction` and `deadline` run Anthropic's server-side web search on Sonnet 5, cite the pages they used, and must close with a **Where to go** link, so county filing fees and deadlines come from the real clerk's page, streamed back as they're written. Search is best-effort — a failure, refusal, or empty answer silently falls back to the no-tools Haiku path with the conservative prompt, so a bad search degrades to vaguer guidance rather than an error. Answers are not verified against the source beyond the model's own citation, and the fallback path can only offer whatever `officialLinks.js` has for that task. The lookup still runs inside a 60s function budget; streaming means an overrun costs the user the tail of an answer rather than the whole request, but it doesn't remove the ceiling. |
...
Baseline e2e tests run on every PR: `sign_in`, `onboarding`, `mark_done`. Feature tests: `calendar_match`, `life_event_new_baby`, `life_event_divorce`, `snooze`, `landing`. PRs that touch a user-facing flow should include a feature test for that flow.
...
3. Wire up real billing (Stripe or similar) and enforce the Mitzy Pro paywall on AI features to match the landing page's pricing claims. Web-search assist is the most expensive per-request feature and is currently ungated — and the per-task `searchAssist` flag now makes it easy to widen that surface, so the paywall should land before it gets used much.
