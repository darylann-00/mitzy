...
| Jurisdiction guidance | `assistType: 'jurisdiction'` tasks resolve the user's real county and state, and the prompt forbids inventing fees, deadlines, phone numbers, or URLs — but the state-level rules themselves still come from model memory, not a live lookup. `/api/assist` calls Claude with no tools. Giving that one assist type the server-side web search tool is the planned fix. |
...
4. Give `assistType: 'jurisdiction'` a real lookup — add the server-side web search tool to `/api/assist` for that type only, so county filing fees and deadlines come from the actual clerk's page instead of model memory. Costs per search and adds latency, so it likely pairs with the Pro tier.
5. The `deadline` assist type is effectively dead — six base-library tasks use it, zero life event tasks. Either fold it into `jurisdiction` or give it the same location treatment.
