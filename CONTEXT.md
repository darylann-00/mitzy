...
| Jurisdiction guidance | Live. `jurisdiction` and `deadline` run Anthropic's server-side web search on Sonnet 5 and cite the pages they used, so county filing fees and deadlines come from the real clerk's page. Search is best-effort — a failure, refusal, or empty answer silently falls back to the no-tools Haiku path with the conservative prompt, so a bad search degrades to vaguer guidance rather than an error. Answers are not verified against the source beyond the model's own citation. |
