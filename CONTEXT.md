...
**2026-04-20:** Audited all task intervals against standard safety guidance. Corrected three tasks in `src/data/tasks.js`: `hm-smoke` (smoke & CO detectors) 180 → 30 days (NFPA 72 requires monthly testing), `hm-fire` (fire extinguisher check) 365 → 30 days (NFPA 10 §7.2.1), `hm-gfci` (GFCI outlets) 365 → 30 days (UL/manufacturer standards). Also tightened `windowDays` and `reminderLeadDays` on all three to fit a monthly cadence. All other intervals were reviewed and are correct.
