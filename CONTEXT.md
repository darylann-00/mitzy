...
- **Landing page (returning vs new)** — `LandingPage` is the first screen on cold launch (before `SlimOnboarding`). Marketing page (hero with category eyebrow + product screenshot, task-chip section, "How it works" 3 steps, benefit cards, "Mitzy and your Google account", pricing — Free vs. Mitzy Pro at $4.99/mo, closing CTA) with two entry points: "Start my free trial" or "Sign in" in the nav. Choice persisted to `WELCOME_CHOICE_KEY` (`mitzy-welcome-v1`) as `'new'` or `'returning'`. `'returning'` skips onboarding and goes straight to `LoginGate` → server profile loads from Supabase into local state. `'new'` keeps the original onboarding → priority-setup → login flow. If a returning user signs in with no server profile (typo, wrong account), `App.js` flips them to `'new'` and routes through onboarding. Welcome key is included in `USER_KEYS`, so reset/sign-out clears it. Pricing tiers are marketing copy only — the app does not yet enforce the Free/Pro split (see "What's Mocked / Incomplete"). The page is also the surface Google's OAuth branding reviewers read, so purpose is stated three ways without turning the page into prose: a "Household task manager" eyebrow pill above the h1, a wrapped row of real task-library chips (HVAC filter, car registration, kids' checkup…) under the heading "The stuff you're supposed to remember" with the plain sentence "Mitzy is a household task manager" as its subhead, and a compact three-line "Mitzy and your Google account" card covering sign-in scope, calendar scope, and read-only/never-shared. Benefit cards reuse the app's own `CategoryIcons` (not emoji) so the page reads as the same product. `index.html` carries the same summary as a plain-HTML `#boot-fallback` block inside `#root` for crawlers and no-JS visitors; an inline head script adds `js-boot` to `<html>` so the block is hidden before it can paint (it used to flash on every load), with a 4s timer that un-hides it if React never mounted. Covered by `tests/e2e/landing.spec.js`.
