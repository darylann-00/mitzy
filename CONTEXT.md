...
- **AssistPanel** — Full-screen overlay. Provider/script/deadline/guidance/guidance_companies modes. Caches 7 days (currently v12). Provider mode uses `ProviderNameSearch` (`src/components/ProviderNameSearch.jsx`, shared with ProfileView's "Saved providers" picker) — a typed name search against `/api/providers` with `skipBlurbs: true` (single Places call, no Claude synthesis), pre-filled with the user's last "good"-voted provider if one exists. Picking a result reveals a "Scheduling" `ProviderCard` with hours, address (links to Google Maps), Call button, and a "Schedule online" button (provider's website) — no AI blurb/rating browsing list anymore. The existing good/bad save flow still writes into `providerHistory`, feeding Profile's "Saved providers" section. `guidance_companies` mode returns JSON with guidance markdown + top 3 national companies (no aggregators); renders `renderMarkdown` + `CompanyCard` rows with external link icon. `renderMarkdown` handles ##headers, bullets, numbered lists with nested sub-bullets, tables, horizontal rules, bold, and auto-linked URLs. `PulseLoader` cycles through 3 contextual messages per `assistType` every 2.5s (not used for providers, which has no loading state).
