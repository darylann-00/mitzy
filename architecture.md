...
- `/api/assist` takes `{ prompt, fallbackPrompt, assistType }`. `assistType` is the only thing that turns on web search, and the server owns that decision — `SEARCH_ASSIST_TYPES` there must stay in sync with `SEARCH_ASSIST_TYPES` in `assistPrompt.js` (which `isSearchAssistType()` exports for the client's cache-TTL, prompt-variant, and response-parsing choices). Search runs on `claude-sonnet-5` with `web_search_20260209`, capped at `MAX_SEARCHES` (3); Haiku 4.5 can't be used because it only supports the older basic search variant.
- **`/api/assist` has two response shapes.** Non-search types return the original one-shot `{ text }` JSON. Search-backed types return a **stream** of line-delimited JSON events — `{type:'status',phase}`, `{type:'text',delta}`, `{type:'reset'}`, `{type:'done'}`, `{type:'error'}`. The client branches on `isSearchAssistType()`; both sides must change together. Rules that fall out of it: the client caches **only** on `done` (a stream that ends without it throws, so a truncated legal answer is never cached for 30 days or shown as finished), `reset` means discard everything streamed so far, and errors after the first byte are in-band rather than HTTP statuses — auth and rate limiting are still checked before the stream opens, so those stay real 401/429s.
